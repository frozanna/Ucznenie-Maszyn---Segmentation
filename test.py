...
test_filenames = glob.glob(
    "input/Drill_test/Cropped_384/kat_30/color_back/img_*.png")
...
for start in tqdm(range(0, len(test_filenames), batch_size)):
    ...
    for id in ids_test_batch:
        img = cv2.imread(
            'input/Drill_test/Cropped_384/kat_30/color/img_{}.png'.format(id))
    ...
    for j in range(9):
        prob = np.array(cv2.resize(preds_keypoints[:, :, j], (orig_width, orig_height))).astype(np.float32)
        cv2.imwrite(
            'input/Drill_test/Cropped_384/kat_30/keypoints/keypoints_{}_{}.png'.format(id, j), prob)
    ...
    for index, pred in enumerate(preds):
        ...
        cv2.imwrite(
            'input/Drill_test/Cropped_384/kat_30/segmantation/mask_{}.png'.format(id), prob)
